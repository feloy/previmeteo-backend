...
class PgProcedures {
  ...
  public function set_timezone ($ts) {
    $this->timezone = $ts;
  }
  ...
  public function __call ($method, $args) {
    if (true || !method_exists($this->_obj, $method)) {
      $pargs = end ($args);
      $limit = NULL;
      $orders = array ();
      $distinct = false;
  ...
  private function search_pg_proc ($method, $args) {
	...
	if ($row['ret_nspname'] == 'pg_catalog' && $row['ret_typtype'] == 'b') { // System scalar type
  ...
  private function cast_value ($rettype, $value) {
	...
	$ret = date ($this->timestamp_return_format, $timestamp);
	return $ret;
      } else
	return NULL;
      break;
  ...
  private function escape_value ($type, $value) {
	...
	$timestamp = mktime($parts['tm_hour'], $parts['tm_min'], $parts['tm_sec'], 
			    $parts['tm_mon']+1, $parts['tm_mday'], $parts['tm_year']+1900);
	$sqlvalue = "'".date ('Y-m-d H:i:s', $timestamp).$this->timezone."'";
      } else {
	$sqlvalue = 'null';
      }
      break;
