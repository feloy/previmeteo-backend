                      ...
                      const double obj_earth, const double earth_sun)
{
   ...
   if( !elem->is_asteroid)
      ...
      if( cos_phase_ang > 1. || cos_phase_ang < -1.)
         printf( "???? Triangle error: %lf %lf %lf\n", obj_sun, obj_earth, earth_sun);
...
FILE *get_mpcorb_file( void)
{
   ...
   if( ifile)
      ...
      n_asteroids = (ftell( ifile) - mpcorb_header_length) / record_length;
...
FILE *get_astorb_file( void)
{
   ...
   if( !ifile)
      ifile = fopen( "d:\\ast\\astorb.dat", "rb");
...
static int get_orbit_rec( char *buff)
{
   ...
   if( using_mpcorb)
      ...
      rval = fread( buff, 2, 1, orbits_file);
      if( buff[1] == 10)         /* single-opp object */
         ...
         bytes_read = 2;
      rval = fread( buff + bytes_read, record_length - bytes_read, 1, orbits_file);
      }
   else
      {
      int i;

      rval = fread( buff, record_length, 1, orbits_file);
      if( rval)
...
AST_DATA *compute_day_data( const long ijd)
{
   ...
   fseek( orbits_file, mpcorb_header_length, SEEK_SET);
...
int main( int argc, char **argv)
{
   ...
   for( n = 0; n < n_ilines; n++)
      if( (!n || memcmp( ilines[n], ilines[n - 1], 12)) &&
         ...
         if( curr_loaded_day_data != (int)jd)
            {
            if( day_data[0])
               free( day_data[0]);
            if( day_data[1])
         ...
         if( ra_motion || dec_motion)
            printf( "\n%s: %.0lf\"/hr in RA, %.0lf\"/hr in dec (%.2lf hours)\n",
         ...
         for( i = (unnumbered_only ? n_numbered : 0); i < n_asteroids; i++)
            ...
            if( is_between( day_data[0][i].ra, day_data[1][i].ra, int_ra, tolerance2 + 5))
               if( is_between( day_data[0][i].dec, day_data[1][i].dec, int_dec, tolerance2 + 5))
                  ...
                  n_checked++;
                  fseek( orbits_file, i * record_length + mpcorb_header_length, SEEK_SET);
                  get_orbit_rec( tbuff);
                  if( using_mpcorb)
                     {
                  ...
                  if( mag < mag_limit && dist < tolerance_in_arcsec)
                     ...
                     if( fabs( computed_dec_motion - dec_motion) < motion_tolerance &&
                        ...
                        sprintf( tbuff + 26, "%6.0lf %6.0lf  %6.0lf  %4.1lf %5.0lf%5.0lf",
                              -ra1 * radians_to_arcsec,
                              -dec1 * radians_to_arcsec, dist,
                              mag, computed_ra_motion, computed_dec_motion);
//                      sprintf( tbuff + strlen( tbuff), "  %.4lf", earth_obj_dist);
                        if( show_lov)
                           sprintf( tbuff + strlen( tbuff), "  %6.0lf",
                        ...
                        if( show_uncertainty)
                           ...
                           if( uncertainty < 0)
                              uncertainty *= -60;
                           sprintf( tbuff + strlen( tbuff), "  %d\"", uncertainty);
                           }
                        for( j = 0; j < n_results
