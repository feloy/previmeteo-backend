...
int main( const int argc, const char **argv)
{
   ...
   while( fgets( buff, sizeof( buff), ifile))
      ...
      if( buff[27] == '.' && buff[58] == '.' && buff[30] == ' ' &&
                                 strlen( buff) > 60)
         {
         int bv_flag, vr_flag, vi_flag, ub_flag;
         ...
         if( bv_flag > 1 && b_minus_v < 98.)
            sprintf( buff + BV_OFFSET, "%5.2lf%c", b_minus_v, bv_flag);
         if( vr_flag > 1 && v_minus_r < 98.)
            sprintf( buff + VR_OFFSET, "%5.2lf%c", v_minus_r, vr_flag);
         if( vi_flag > 1 && v_minus_i < 98.)
            sprintf( buff + VI_OFFSET, "%5.2lf%c", v_minus_i, vi_flag);
