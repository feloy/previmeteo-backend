...
static int moonvisibility( double angle)
{
   angle = fmod( angle, 2. * pi);
   if( angle < 0.)
      angle += 2. * pi;
   return( (int)( angle * 100. / 2. / pi));
}
   ...
   for( month = month_start; month <= month_end; month++)
      ...
      for( i = 0; i < (int)( jd_end - jd_start); i++)
	{
	  for (int h=0; h<24; h++) {
	    ...
	    double jd = (double)( jd_start + i) - 1.5 + h/24. - (double)time_zone / 24.;
	    char buff[80];
	    int j, quad0, quad1;
		...
		printf ("%d-%02d-%02d %02d:%02d+00\t", year, month, i+1, minutes / 60, minutes % 60);
	      }
#endif
	    ...
	    int visib = moonvisibility (lunar_lon - solar_lon);
	    if (visib % 25 == 0) {
	      printf ("%d-%02d-%02d %d\n", year, month, i+1, visib);
	      i++;
	      break;
	    }
